...
inst returns[AbstractInst tree]
    ...
    | PRINT OPARENT list_expr CPARENT SEMI {
            ...
            $tree = new Print(false, $list_expr.tree); // false to not return to the line
...
if_then_else returns[IfThenElse tree] @init {
    LinkedList<ListInst> nestedLists = new LinkedList<>();
    ...
    ListInst elseBranch = new ListInst();
        ...
        nestedLists.add(currentList);
    }
    (ELSE elsif=IF OPARENT elsif_cond=expr CPARENT OBRACE elsif_li=list_inst CBRACE {
        assert($elsif_cond.tree != null);
        assert($elsif_li.tree != null);
        currentList = new ListInst();
        IfThenElse nestedIf = new IfThenElse($elsif_cond.tree, $elsif_li.tree, currentList);
        nestedLists.getLast().add(nestedIf);
...
literal returns[AbstractExpr tree]
    : INT {
         try{
            ...
            $tree = null;
            throw new DecaRecognitionException("Invalid int : " + $INT.text +  " is not valid (out of range or malformed)", this, $ctx);
            }
    ...
    | fd=FLOAT {
        try {
            ...
            $tree = null;
            throw new DecaRecognitionException("Invalid float : " + $fd.text +  " is not valid (out of range or malformed)", this, $ctx);
            }
    }
    | STRING {
        String value = $STRING.text;
        $tree = new StringLiteral(value.substring(1, value.length() - 1).replace("\\\"", "\"").replace("\\\\", "\\"));
...
class_decl returns[AbstractDeclClass tree]
    : CLASS name=ident superclass=class_extension OBRACE body=class_body CBRACE {
        ...
        assert($body.tree != null);
        AbstractIdentifier superClass = $superclass.tree;
        if (superClass == null) {
            superClass = new Identifier(getDecacCompiler().createSymbol("Object"));
        }
        $tree = new DeclClass($name.tree, superClass, $body.tree.getFields(), $body.tree.getMethods());
...
class_extension returns[AbstractIdentifier tree]
    ...
    | /* epsilon */ {
            Identifier object = new Identifier(getDecacCompiler().createSymbol("Object"));
            object.setLocation(Location.BUILTIN);
            $tree = object;
        }
    ;
class_body returns [ListDeclClassBody tree]
@init {
    $tree = new ListDeclClassBody();
    ...
    | f=decl_field_set {
        ...
        $tree.setFields(fields);
        $tree.setMethods(methods);
    }
    ;
...
@init {
    ...
    : v=visibility t=type list_decl_field[$t.tree, $v.tree] SEMI {
        assert($list_decl_field.tree != null);
        for (AbstractDeclField field : $list_decl_field.tree.getList()) {
            $tree.add(field);
        }
    }
    ;
...
list_decl_field[AbstractIdentifier typeIdent, Visibility vis] returns [ListDeclField tree]
@init {
    $tree = new ListDeclField();
}
    : dv1=decl_field[$typeIdent, $vis] {
        assert($dv1.tree != null);
        $tree.add($dv1.tree);
    }
    (COMMA dv2=decl_field[$typeIdent, $vis] {
...
decl_field[AbstractIdentifier typeIdent, Visibility vis] returns [AbstractDeclField tree]
    : i=ident {
        assert($i.tree != null);
        $tree = new DeclField($typeIdent, $i.tree, new NoInitialization(), $vis);
        setLocation($tree, $i.start);
    }
    (EQUALS e=expr {
        assert($e.tree != null);
        $tree = new DeclField($typeIdent, $i.tree, new Initialization($e.tree), $vis);
        setLocation($tree, $i.start);
    }
    )?
    ;
...
@init {
    ...
    : t=type i=ident OPARENT lp=list_params CPARENT (block {
                assert($block.decls != null);
                assert($block.insts != null);
      ...
      | ASM OPARENT code=multi_line_string CPARENT SEMI {
        ...
        assert($i.tree != null);
        assert($lp.tree != null);
        $tree = new DeclMethod($t.tree, $i.tree, $lp.tree, body);
