 ...
 * 
 * @author gl13, Based on template by Jim Idle - Temporal Wave LLC
 *         (jimi@idle.ws)
 * @date 01/01/2025
 */
public abstract class AbstractDecaLexer extends Lexer {
    ...
    /**
     * Validates whether a string represents a valid IEEE-754 single-precision
     * floating-point literal.
     *
     * @param text the string to validate
     * @throws DecaRecognitionException if the string is not a valid float or
     *                                  if its value is out of range for single
     *                                  precision.
     */
    public void validateFloatLiteral(String text) throws DecaRecognitionException {
        try {
            float value = Float.parseFloat(text);

            // Vérify  limits of IEEE-754 simple precision
            if (Float.isInfinite(value)) {
                throw new DecaRecognitionException(
                        "Float value too large: " + text,
                        this,
                        _input
                );
            }

            if  (value == 0.0f && text.matches(".*[^0].*")) {
                throw new DecaRecognitionException(
                        "Float value too small: " + text,
                        this,
                        _input);
            }
        } catch (NumberFormatException e) {
            throw new DecaRecognitionException(
                    "Invalid float format: " + text,
                    this,
                    _input);
        }
    }
